...
/*    
	void TimeAlarmsClass::showAlarm(AlarmID_t id)
    {
	   printf("Id %d ", id);
        if(isAllocated(id) && Alarm[id].Mode.isEnabled ){
		   printf("is %soneshot, type=%d, day=%d h=%d: m=%d\n",
		           Alarm[id].Mode.isOneShot ? "" : "not ",	       
    	            Alarm[id].Mode.alarmType,
    	            day(Alarm[id].value),
					hour(Alarm[id].value),
				    minute(Alarm[id].value) );
   
		}
		else {
		  printf("Id not allocated or not enabled\n");
		}
    }	
*/	
