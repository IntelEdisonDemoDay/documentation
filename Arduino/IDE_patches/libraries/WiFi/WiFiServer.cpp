...
void WiFiServer::begin()
{
	...
	if (_sock < 0){
		Serial.println("unable to open a TCP socket!");
		return;
	}
	...
	// This is more complicated than it maybe needs be, because
	// sometimes (esp. during testing) when you restart the Arduino
	// sketch it will get here before the system has fully released
	// the listening port from last time.
        int tries = 0;
        do
        {
		...
		tries++;
		if (errno == EADDRINUSE) {
			// If we get here, either something else is using
			// the port, or a previous instance of ourselves is.
			// In case it's the latter, wait briefly and retry
        ...
        while ((tries < 50) && (errno == EADDRINUSE) && (ret < 0));
	if (tries >= 50) {
		// We're giving up!
		return;
	}
